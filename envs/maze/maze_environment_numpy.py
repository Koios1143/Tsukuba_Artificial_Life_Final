...
class MazeEnvironment:
    def __init__(self, init_location, walls, exit_point, init_heading=180, exit_range=5.0, agent_kwargs={}):
        ...
        self.LENGTH = 1005
        self.WIDTH = 1005
        self.visited = [[0 for width in range(self.WIDTH)] for length in range(self.LENGTH)]
        self.travel_unvisited_cnt = 0
    ...
    def update(self, control_signals):
        ...
        cur_x, cur_y = self.agent.location
        cur_x, cur_y = int(cur_x), int(cur_y)
        if self.visited[cur_x][cur_y] == 0:
            self.travel_unvisited_cnt += 1
            # print(f'Updating trabel unvisited count to {self.travel_unvisited_cnt}')
            try:
                self.visited[cur_x][cur_y] = 1
            except Exception as e:
                print(f'Error when setting visited {(cur_x, cur_y)} with error: {e}')
